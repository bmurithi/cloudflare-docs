...
For quick preview deployments we recommend using [Cloudflare Tunnel](/cloudflare-one/connections/connect-networks/) to generate preview URLs to your web services. These work across local development, workers.dev and production usage.
...
We are working on production deployments, custom hostnames, and authentication for Cloudflare Tunnel support. In the mean time we recommend using `exposePort()` and `proxyToSandbox()` documented in [Production usage, stable URLs and custom domains](#production-usage-stable-urls--custom-domains).
:::
